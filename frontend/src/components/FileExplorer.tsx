...
export default function FileExplorer({
  ...
  const [pendingDecryptFile, setPendingDecryptFile] = useState<FileMetadata | null>(null)
  ...
  // Initial Click -> Check enrollment and open Face Auth Modal
  const handleDecryptClick = (file: FileMetadata) => {
    if (!isEnrolled) {
      alert("⚠️ Biometric Identity Not Registered!\n\nPlease register your identity first by clicking the 'Register Identity' button in the header.")
      return
    }
    setPendingDecryptFile(file)
  }

  // Actual Decryption -> Called after Face Auth Success
  const performDecryption = async () => {
    if (!pendingDecryptFile) return
    const file = pendingDecryptFile
    setPendingDecryptFile(null) // Close modal

    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/files/${file.file_id}/decrypt`,
        {
          headers: {
            'X-User-ID': userId
          }
      ...
      a.href = url
      a.download = `decrypted_${file.filename}`
      ...
      alert('Failed to decrypt file. Access Denied or System-Fault.')
    }
  }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      {/* Face Auth Modal */}
      <FaceAuthModal
        isOpen={!!pendingDecryptFile}
        onClose={() => setPendingDecryptFile(null)}
          ...
          <div className="divide-y">
            {files.length === 0 ? (
              ...
              files.map((file) => (
                <div
                  key={file.file_id}
                  className={`p-4 hover:bg-gray-50 cursor-pointer transition-colors ${selectedFile?.file_id === file.file_id ? 'bg-blue-50' : ''
                    }`}
                    ...
                    <div className="flex">
                      {file.encryption_status === 'encrypted' && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            handleDecryptClick(file)
                          }}
                          className="ml-2 p-2 text-gray-400 hover:text-green-600"
